...
// REVISION: Proprioceptive Eye Integration & Lunar Evacuation
...
window.addEventListener('q-tick', (e) => {
    ...
    const elDebt = document.getElementById('metro-debt');
    if (elTau) elTau.innerText = `${tauOffset > 0 ? '+' : ''}${tauOffset}h`;
    if (elDebt) {
        elDebt.innerText = tensionScore > 70 ? "HIGH" : "NOMINAL";
        elDebt.style.color = tensionScore > 70 ? "var(--warn-orange)" : "var(--theme-main)";
    }
    ...
    if (elHorizon) elHorizon.innerText = `T-${Math.floor(remainingMins)}M`;
