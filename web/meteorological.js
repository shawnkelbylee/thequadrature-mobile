...
// REVISION: Eradicated Cardinal Earths. Initialized Y-Axis Sine Translation & 24H Raster Rotation.
...
function updateGlobeKinematics() {
    ...
    // At Northern Solstice, sine is 1, Globe translates upwards (negative Y).
    const maxTiltPX = 35; 
    ...
    // 2. DIURNAL ROTATION (The Spin)
    const hours = now.getHours();
    const minutes = now.getMinutes();
    const seconds = now.getSeconds();
    
    // 24-hour fractional progression (0.0 to 1.0)
    const timeFraction = (hours + (minutes / 60) + (seconds / 3600)) / 24;
    
    // Background mapping (100% is one full planetary rotation)
    const bgX = timeFraction * 100;
    
    const surface = document.getElementById('diurnal-surface');
    if(surface) {
        surface.style.backgroundPosition = `${bgX}% 0`;
    }
}
